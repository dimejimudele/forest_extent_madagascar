...
var from_glc_image = ee.Image(
  'projects/ee-mudeledimeji/assets/postdoc/deforestation_mdg_paper/rasters/from_glc_landcover/FROM_GLC_yearImg_2020'
  ).clip(roi);
...
var esri_image = ee.Image(
  'projects/ee-mudeledimeji/assets/postdoc/deforestation_mdg_paper/rasters/esri_landcover/esri_mdg_2020'
  ).clip(roi);
...
// Create Copernicus - ESA tree class layer 

var esa_image = ee.ImageCollection("ESA/WorldCover/v100")
                                .select(["Map"])
                                .filterBounds(roi)
                                .first()
                                .clip(roi);

var esa_image_trees = esa_image.eq(10);

var esa_image_trees_30m = esa_image_trees
  .reproject({
    crs: crs,
    scale: 30
  });

esa_image_trees_30m = esa_image_trees_30m.unmask(0).rename("mask");
print("ESA data", esa_image_trees_30m);
...
Export.image.toDrive({
  ...
  description: "agreement_map_2020_30m", // Get the export name as a plain JavaScript string
